...
class PathFinder {
	...
	// as in clojure
	class PathNode {
		public PathNode(MotionPath nodeValue) {
	...
	// Overload with connectedThreshold being 0, only considers MotionPaths with
	// equal begin/end-points neighbours
	...
	// connectedThreshold: threshold for considering other MotionPaths
	// neighbours
	public MotionPath[] find(MotionPath[] routes, float connectedThreshold) {
		...
		while (openNodes.size() != 0) {
			for (PathNode p : getNeighbours(currentNode, openNodes,
					connectedThreshold)) {
	...
	private List<PathNode> getNeighbours(PathNode node, List<PathNode> routes,
