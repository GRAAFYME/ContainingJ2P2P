...
public class MotionPaths {
	...
	public MotionPath path, path2, path3, path4, StartingPoint,
			CrossStartPoint, toSeaCrane, CrossingToTrucks,
			CrossingToBargeCrane, OtherSidePlatform, SeaCraneToOtherSide,
			OtherSideToAGV, OtherWayAround, CrossingStartToTruck,
			AGVStartToOtherSide, CrossingToTrains, CrossingToStorageSide1,
			CrossingToStorageCraneSide1, CrossingToStorageSide2,
			CrossingToStorageCraneSide2, CrossingToTruckCrane;
	...
	public List<MotionPathProtocol> getMotionPathProtocols() {
		...
		for (Map.Entry<String, MotionPath> e : hmMotionPaths.entrySet()) {
			MotionPathProtocol newPath = new MotionPathProtocol(
					getWaypointsJavax(e.getValue()), e.getValue().getLength(),
					e.getKey());
	...
	public void StartPoint() {
		...
		for (int i = 0; i < 50; i++) {
			...
			hmMotionPaths.put("StartingPoint" + String.valueOf(i),
					StartingPoint);
			x += 25;
		}
		x = -470f;
		for (int j = 0; j < 50; j++) {
			...
			StartingPoint.enableDebugShape(assetManager, AllAgvNodes);
			hmMotionPaths.put("StartingPoint" + String.valueOf(j),
					StartingPoint);
			x += 25;
		}
	}
	...
	public void CrossingStart() {
		...
		for (int j = 0; j < 50; j++) {
			...
			CrossStartPoint.enableDebugShape(assetManager, AllAgvNodes);
			hmMotionPaths.put("CrossStartPoint" + String.valueOf(j),
					CrossStartPoint);
			x += 25;
		}
	...
	public void CrossingStartToTruck() {
		for (int i = 0; i < 20; i++) {
			...
			hmMotionPaths.put("CrossingStartToTruck" + String.valueOf(i),
					CrossingStartToTruck);
		}
	}
	...
	public void CrossingToStorageCraneSide1() {
		...
		for (int i = 0; i < 24; i++) {
			...
			CrossingToStorageCraneSide1.enableDebugShape(assetManager,
					AllAgvNodes);
			hmMotionPaths.put(
					"CrossingToStorageCraneSide1" + String.valueOf(i),
					CrossingToStorageCraneSide1);
			x += 60;
		}
	}
	...
	public void CrossingToStorageCraneSide2() {
		...
		for (int i = 0; i < 24; i++) {
			...
			CrossingToStorageCraneSide2.enableDebugShape(assetManager,
					AllAgvNodes);
			hmMotionPaths.put(
					"CrossingToStorageCraneSide2" + String.valueOf(i),
					CrossingToStorageCraneSide2);
			x += 60;
		}
	}
	...
	public void CrossingToTruckCrane() {
		...
		for (int i = 0; i < 20; i++) {
			...
			hmMotionPaths.put("CrossingToTruckCranes" + String.valueOf(i),
					CrossingToTruckCrane);
			x += 40;
		}
	}
