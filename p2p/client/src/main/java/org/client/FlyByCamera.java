...
public class FlyByCamera implements AnalogListener, ActionListener {
	...
	public FlyByCamera(Camera cam, InputManager inputManager) {
		...
		cam.setRotation((RotateCam.fromAngleAxis(FastMath.HALF_PI,
				Vector3f.UNIT_Y)));
	...
	public void registerWithInput() {
		String[] mappings = new String[] { "FLYCAM_RiseSpace",
				"FLYCAM_LowerShift" };
		inputManager.addMapping("FLYCAM_RiseSpace", new KeyTrigger(
				KeyInput.KEY_SPACE));

		inputManager.addMapping("FLYCAM_LowerShift", new KeyTrigger(
				KeyInput.KEY_LSHIFT));
