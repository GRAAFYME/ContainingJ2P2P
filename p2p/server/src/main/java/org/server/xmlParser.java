...
public class xmlParser {
	public static void main(String[] Args) {
		...
		System.out.println(vehicles.peek().containers.size()
				+ ", vehicle #1 contains this many containers");
	}
	...
	public ContainerSetXml load(String path) throws FileNotFoundException {
		...
		try {
			...
			JAXBContext jaxbContext = JAXBContext
					.newInstance(ContainerSetXml.class);
	...
	public PriorityQueue<Vehicle> parse(ContainerSetXml containerSet) {
		...
		for (ContainerXml c : containerSet.containers) {
			String uniqueTransportArrivalKey = c.arrivalTransportType
					+ c.arrivalCompany + c.arrivalDay + c.arrivalFrom
					+ c.arrivalMonth + c.arrivalTill + c.arrivalYear;
		...
		PriorityQueue<Vehicle> queue = new PriorityQueue<Vehicle>(2,
				new Comparator<Vehicle>() {
					public int compare(Vehicle v1, Vehicle v2) {
						...
						int comparison1 = Integer.compare(c1.arrivalDay,
								c2.arrivalDay);
						int comparison2 = Integer.compare(c1.arrivalMonth,
								c2.arrivalMonth);
						int comparison3 = Integer.compare(c1.arrivalYear,
								c2.arrivalYear);

						int comparison4 = c1.arrivalFrom
								.compareTo(c2.arrivalFrom);
	...
	private List<ContainerXml> removeNrDuplicates(List<ContainerXml> containers) {
		...
		System.out.println(containers.size() - returnList.size()
				+ " duplicate containers(Nr) removed");
		return returnList;
	}
}
